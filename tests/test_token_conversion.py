...
def test_lusd_to_dai_swap(test_strategy, dai, lusd, lusd_whale):
    assert lusd.balanceOf(test_strategy) == 0
    assert dai.balanceOf(test_strategy) == 0

    lusd.transfer(test_strategy, 1_000 * (10 ** dai.decimals()), {"from": lusd_whale})
    test_strategy.sellLUSDforDAI(lusd.balanceOf(test_strategy))

    print(f"Swapped 1000 LUSD for {dai.balanceOf(test_strategy)/1e18:.2f} DAI")
...
def test_dai_to_lusd_swap(test_strategy, dai, dai_whale, lusd):
    ...
    test_strategy.sellDAIforLUSD(dai.balanceOf(test_strategy))
...
def test_claim_rewards_ends_in_dai(
    test_strategy, accounts, dai, lqty, lqty_whale, weth, lusd
):
    ...
    print(
        f"Swapped 10 ETH and 1000 LQTY for {dai.balanceOf(test_strategy)/1e18:.2f} DAI"
    )
